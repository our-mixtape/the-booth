...
September 10, 2026 hosted verification: deployed the existing gateway on the user-approved Render service and connected the existing Vercel app, using the matching Booth credentials with explicit transfer authorization. No dependency or charity-jukebox infrastructure was copied or changed. Added original capture-time rectangle handling to the Swift helper to exclude surrounding private content before encoding. New hosted video/audio and screenshots use only original fixtures, real Clerk sign-in and genuine gpt-6-astra output. System-audio metrics establish digital capture, not human audition; provider-failure tests remain simulated. A temporary user-approved Vercel rehearsal link was revoked after testing and its token is absent from shared artifacts.
